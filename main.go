...
func main() {
	...
	zee := createGroup()
	if api {
		go startAPIServer(apiAddr, zee)
	}
	startCacheServer(addrMap[port], []string(addrs), zee)
...
func startCacheServer(addr string, addrs []string, gee *cache.Group) {
	...
	peers.Set(addrs...)
	gee.RegisterPeers(peers) //peers是HTTPPool类型，但是实现了PeerPicker接口的方法PickPeer，这里就可以传进去
